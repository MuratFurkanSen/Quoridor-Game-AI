...
class Agent:
    def __init__(self, name, pawn, opponent, activeWalls):
        ...
        self.gamma = 0.9
        self.startLevel = 0
        self.model = QLinearNet(name, 68, 16, 32, 16, 128)
    ...
    def train_long_memory(self):
        if len(self.memory) > BATCH_SIZE:
            ...
            sample = self.memory
        # List of Each One
        state, action, reward, next_state, done, next_mask = zip(*sample)
        self.trainer.train_step(state, action, reward, next_state, done, next_mask)
...
def train():
    ...
    while True:
        ...
        # Get Move
        move = playerAgent.get_action(old_state, game.epsilonStart) if game.playerTurn \
            else opponentAgent.get_action(old_state, game.epsilonStart)
        ...
        if done:
            ...
            if playerScore > playerRecord or playerAgent.n_games % 5 == 0:
                ...
                playerAgent.model.save(f"Player_Model_Wall-{playerAgent.n_games}.pth")
            if opponentScore > opponentRecord or opponentAgent.n_games % 5 == 0:
                opponentRecord = opponentScore
                opponentAgent.model.save(f"Opponent_Model_Wall-{opponentAgent.n_games}.pth")
            ...
            opponentMeanScore = opponentTotalScore / opponentAgent.n_games
            ...
            if game.exit:
                ...
                }, 'model_saves/Wall_Player_Model.pth')
                ...
                torch.save({
                    ...
                    'level': game.level,
                    'n_games': playerAgent.n_games,
                }, 'model_saves/Wall_Opponent_Model.pth')
                quit()
